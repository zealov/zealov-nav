...
;(function() {
	...
	var handleSubmit = function(editor, info) {
		return function(api) {
			...
			DialogConfirms.preprocess(editor, changedData).then(function(
				pData
			) {
	...
	var getInitialData = function(info, defaultTarget, editor) {
		const fileList = editor.getParam('fileList')
		var anchor = info.anchor
		var url = anchor.url.getOr('')
		let value = '0'
		let newUrl = url
		if (url.indexOf('src=') > -1) {
			const urlArr = url.split('src=')
			if (urlArr.length > 1) {
				newUrl = decodeURIComponent(urlArr[1])
			}
		}
	...
	var makeDialog = function(settings, onSubmit, editor) {
		const fileList = editor.getParam('fileList')
		const webUrl = editor.getParam('webUrl')
		const items = [
			{
			...
			},
			{
				...
				label: '显示文字', // text for the label
			},
			{
				type: 'input', // component type
				name: 'title', // identifier
				label: '标题', // text for the label
			},
			{
			...
			},
			{
				...
				label: '启用文档在线预览', // text for the label
			},
		]
		var defaultTarget = Optional.from(getDefaultLinkTarget(editor))
		var initialData = getInitialData(settings, defaultTarget, editor)
		...
		return {
			...
			onChange(api, details) {
				...
				if (status && newUrl.value) {
					newUrl.value = `${webUrl}?src=${encodeURIComponent(
						newUrl.value
					)}`
				} else {
					const { value } = newUrl
					if (value) {
						const urlArr = value.split('src=')
						if (urlArr.length > 1) {
							newUrl.value = decodeURIComponent(urlArr[1])
						}
					}
				}
				api.setData({
	...
	var toggleUnlinkState = function(editor) {
		return function(api) {
			var hasLinks$1 = function(parents) {
				return (
					hasLinks(parents) ||
					hasLinksInSelection(editor.selection.getRng())
				)
			}
	...
	var setupButtons = function(editor) {
		editor.ui.registry.addToggleButton('uploadfile', {
			icon: 'link',
	...
	var setupContextMenu = function(editor) {
		var inLink = 'uploadfile unlink openlink'
		var noLink = 'uploadfile'
	...
	var setupContextToolbars = function(editor) {
		...
		editor.ui.registry.addContextForm('quicklink', {
			...
			predicate: function(node) {
				return (
					!!getAnchorElement(editor, node) &&
					hasContextToolbar(editor)
				)
			},
			initValue: function() {
			...
			commands: [
				{
					...
					onAction: function(formApi) {
						...
						if (!anchor) {
							...
							var text = onlyText
								? Optional.some(
										getAnchorText(editor.selection, anchor)
								  )
										.filter(function(t) {
											return t.length > 0
										})
	...
	function Plugin() {
		global.add('uploadfile', function(editor) {
			setupButtons(editor)
