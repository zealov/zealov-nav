...
class FileSystem
{
    ...
    {
        $path = $this->getDisk()->putFile($this->getDirectory(), $request->file($key));

        if($request->get('thumbnail') == 'open' ){
            Image::make(storage_path('app/public/'.$path))
                ->resize($this->thumbnailWidth??500, null, function($constraint){       // 调整图像的宽到300，并约束宽高比(高自动)
