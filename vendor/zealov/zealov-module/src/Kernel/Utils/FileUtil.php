...
class FileUtil
{
    ...
    public static function safePath($path, $permit = ['public/temp', 'public/data'])
    {
        $path = realpath($path);
        if (empty($path)) {
            ThrowException::throws('FileSafePath File Not Exists');
        }
        $whiteList = [];
        foreach ($permit as $p) {
        ...
        if (!$found) {
            ThrowException::throws('FileSafePath Not Permit');
        }
        return $path;
    }
    ...
    public static function generateLocalTempPath($ext = 'tmp')
    {
        ...
        ThrowException::throws('FileUtil generateLocalTempPath error');
    }
